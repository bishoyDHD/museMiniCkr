...
Long_t GEM_BH::process()
{
    ...
    //runs for 161+ BM
    // double startmu[numbars] = {17,17,18,17};
    // double stopmu[numbars] = {20,20,1,20};

    // double startpi[numbars] = {7,7,8,7};
    // double stoppi[numbars] = {12,12,13,12};

    // double barcut[numbars] = {0,1,2,3};

    // double starte[numbars] = {1,0,2,0};
    // double stope[numbars] = {4,4,5,3};

    // runs for 161+ BM no IFP GEMs
    // double startmu[numbars] = {17};
    // double stopmu[numbars] = {20};

    // double startpi[numbars] = {7};
    // double stoppi[numbars] = {12};

    // double barcut[numbars] = {1};

    // double starte[numbars] = {1};
    // double stope[numbars] = {4};
    ...
    for (size_t j = 0 ; j < GEM_Tracks->tracks.size() ; j++)
    {
        ...
        for(size_t i = 0; i < Hits->hits.size(); i++)
        {
                    ...
                    for(int k=0;k<numbars;k++)
                    {
                        if(plane==3 && bar==barcut[k] && RF > startmu[k] && RF < stopmu[k])
                        {
                            ...
                            if(z==-250)
                            {
                                ...
                                H2(xtarg,xangle,"mu/xp V xtarg","mu xp V xtarg;xtarg;xp",400,-75,75,200,-30,30);
                                H2(ytarg,yangle,"mu/yp V ytarg","mu yp V ytarg;ytarg;yp",400,-75,75,200,-30,30);
                                ...
                                if(my!=-1e4&&y0!=-1e4&&myifp!=-1e4&&y4!=-1e4)
                                {
                                    ...
                                    H1(xangle,"Turtle/angles/xp","xp",100,-100,100);
                                    H1(yangle,"Turtle/angles/yp","yp",100,-100,100);
                                    H1(xifpangle,"Turtle/angles/xifp","xifp",100,-100,100);
                                    H1(yifpangle,"Turtle/angles/yifpp","yifp",100,-100,100);
                                }
                                cd("..");
                            }
                        ...
                        if(plane==3 && bar==barcut[k] && RF > starte[k] && RF < stope[k])//&& mxifp > -0.005 && mxifp < 0.00 && xifp >0 && xifp <2 )
                        {
                            ...
                            if(z==-250)
                            {
                                ...
                                H2(xtarg,xangle,"e/xp V xtarg","e xp V xtarg;xtarg;xp",400,-75,75,200,-30,30);
                                H2(ytarg,yangle,"e/yp V ytarg","e yp V ytarg;ytarg;yp",400,-75,75,200,-30,30);
                                ...
                                if(my!=-1e4&&y0!=-1e4&&myifp!=-1e4&&y4!=-1e4)
                                {
                                    ...
                                    H1(yangle,"Turtle/angles/yp","yp",100,-100,100);
                                    H1(xifpangle,"Turtle/angles/xifp","xifp",100,-100,100);
                                    H1(yifpangle,"Turtle/angles/yifp","yifp",100,-100,100);
                                }
                                cd("..");
                            }
                        ...
                        if( plane==3 && bar==barcut[k] && RF > startpi[k] && RF < stoppi[k])//&& mxifp > -.005 && mxifp < 0.00 && xifp >0 && xifp <2)
                        {
                            ...
                            if(z==-250)
                            {
                                ...
                                H2(xtarg,xangle,"pi/xp V xtarg","pi xp V xtarg;xtarg;xp",400,-75,75,200,-30,30);
                                H2(ytarg,yangle,"pi/yp V ytarg","pi yp V ytarg;ytarg;yp",400,-75,75,200,-30,30);
        	...
        	if(x0!=-1e4 && y0!=-1e4 && ((plane==3 && bar==4)))// || (plane==3 && bar==10)) )
        		H2(x0,y0,"US GEM x V y cut on BH","US GEM x V y",500,-50,50,1000,-50,50);
        	if(x1!=-1e4 && y1!=-1e4&& ((plane==3 && bar==4)))// || (plane==3 && bar==10)))
        		H2(x1,y1,"4th GEM x V y cut on BH","4th GEM x V y",500,-50,50,1000,-50,50);
        	if(x2!=-1e4 && y2!=-1e4 &&((plane==3 && bar==4)))// || (plane==3 && bar==10)))
        		H2(x2,y2,"MI GEM x V y cut on BH","MI GEM x V y",500,-50,50,500,-50,50);
        	if(x3!=-1e4 && y3!=-1e4 && ((plane==3 && bar==4)))// || (plane==3 && bar==10)))
        		H2(x3,y3,"DS GEM x V y cut on BH","DS GEM x V y",500,-50,50,500,-50,50);
		}
