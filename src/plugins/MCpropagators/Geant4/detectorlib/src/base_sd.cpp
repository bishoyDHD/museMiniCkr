//! \file
//!
//! Source file for Basic sensitiv detector
//!
...
G4bool base_SD::ProcessHits( G4Step * step, G4TouchableHistory * /*ROhist*/ ) {
  ...
  hit.track = track->GetTrackID();
   ...
   hit.fourmomentum=TLorentzVector(mom.x()/MeV,mom.y()/MeV,mom.z()/MeV,0.5*(prestep->GetTotalEnergy()+poststep->GetTotalEnergy())/MeV);
